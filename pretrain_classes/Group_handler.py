...
class Filled_Lot():
    ...
    def __init__(self, car_group):
        ...
        for i in range(n):
            ...
            car1 = Static_car( (x + dx ,y + 1.2*h) )
            car2 = Static_car( (x - dx ,y + 1.2*h) )
            ...
            dx += delta
            if i%2 == 0:
                car1 = Static_car((x + dx ,y - 1.2*(h+w)*0.5), vertical=False)
                car2 = Static_car((x - dx ,y - 1.2*(h+w)*0.5), vertical=False)          
                self.static_cars_list.append(car1)
                self.static_cars_list.append(car2)            
            dx += delta
            if i == n-2:
